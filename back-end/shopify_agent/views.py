...
def openclaw_response_receiver(request):
    ...
    try:
        ...
        user_msg = payload.get('body') or payload.get(
            'text') or payload.get('message', '')
        user_msg = user_msg.strip() if user_msg else ""

        user_id = payload.get('sender_id') or payload.get(
            'from') or payload.get('sender', 'default')

        # 1. Detección de Confirmación Determinista
        is_confirmation = any(word in user_msg.lower() for word in [
                              'si', 'sí', 's', 'confirmar', '/confirm_order'])

        if is_confirmation:
            print(
                f"--- [ROUTER] Intento de procesamiento determinista para {user_id} ---")
            # Buscar alerta pendiente
            alert = LowStockAlert.objects.filter(
                status='notified').order_by('-created_at').first()
            ...
            if alert:
                ...
                if provider and provider.email:
                    print(
                        f"--- [ROUTER] Ejecutando envío directo a {provider.email} ---")
                    """
                    #new
                    return JsonResponse({
                        "status": "ok",
